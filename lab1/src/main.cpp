...
//zle zrozumialem polecenie ;( ale co sie pobawilem C++ em to moje xd)
struct Timer
{
...
void getGraphDataFromFile(const std::filesystem::path& filePath, std::vector<Vertex>& vertices, int startLine = 9)
{
    ...
    while (!fileStream.eof())
    {
        ...
        if (currentLineCount >= startLine)
        {
            ...
            {
                //std::cout << "V = " << vertexNumber << " | cord  = (" << x << ", " << y << ")\n";
...
std::unique_ptr<Graph> createCompleteGraph(std::vector<Vertex>& vertices)
{
    ...
    {
        // std::cout << "V = " << vertices[i].getNumber() << " | cord  = (" << vertices[i].getX() << ", " << vertices[i].getY() << ")\n";
...
int test(std::vector<Vertex>& vertices, std::mt19937& mt, std::uniform_int_distribution<int>& randomVertex, int noOfGroups, int noOfRepeats, int& minValue)
{
    ...
    for (int i {0}; i < noOfRepeats; ++i)
    {
        ...
        for (int j {0}; j < noOfGroups; ++j)
        {
            ...
            // std::cout << "Permutation weight: " << permutationCycleWeight << "\n";
...
int main() {
    ...
    {       
        ...
        std::cout << "TEST 1 [50 repeats] [20 groups] Average min " << test(vertices, mt, randomVertex, 20, 50, MIN_VALUE_20_GROUPS) << "\n"; 
