...
# Directories
vertices_directory = 'res/verticesData'
# edges_directory = 'res/MSTEdges'
edges_directory = 'res/DFSCycleEdges'
...
for file in os.listdir(vertices_directory):
    if file.endswith(".tsp"):
        ...
        nx.draw(G, pos, with_labels=False, node_size=30, node_color='red', font_weight='bold')

        plt.suptitle(f'DFS Cycle for {os.path.splitext(os.path.basename(file.name))[0]}')  # Set title for the current file

        plt.grid(True)
        plt.savefig(f'res\\plots\\DFSPlots\\DFS_cycle_plot_{os.path.splitext(os.path.basename(file.name))[0]}.png')
        plt.close()